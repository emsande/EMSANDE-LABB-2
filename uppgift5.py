...
def main():
    ...
    def cirkel(center: tuple[float, float], radie, color: str="#ffffff"):
        x2 = center[0]+radie
        x1 = center[0]-radie
        y2 = center[1]+radie
        y1 = center[1]-radie
        radie=normalized_to_pixel(radie)
        for y in range(normalized_to_pixel(y1), normalized_to_pixel(y2)):
            for x in range(normalized_to_pixel(x1), normalized_to_pixel(x2)):
                Area = (x-normalized_to_pixel(center[0]))**2 + (y-normalized_to_pixel(center[1]))**2 
                if Area <= radie**2: 
                    img.put(color, (x,y))

    cirkel((0.3,0.5), 0.1,"#f67f89")
