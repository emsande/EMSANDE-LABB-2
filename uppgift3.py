...
def main():
    ...
    def ram(upper_left: tuple[float, float], lower_right: tuple[float, float], tjocklek, color: str="#ffffff"):

     for y in range(normalized_to_pixel(upper_left[1]), normalized_to_pixel(lower_right[1])):
        for x in range(normalized_to_pixel(upper_left[0]), normalized_to_pixel(lower_right[0])):
          if x < normalized_to_pixel (upper_left[0] + tjocklek*1/WIDTH) or \
             x > normalized_to_pixel (lower_right[0] - tjocklek*1/WIDTH - 1/WIDTH) or \
             y < normalized_to_pixel (upper_left[1] + tjocklek*1/WIDTH) or \
             y > normalized_to_pixel (lower_right[1] - tjocklek*1/WIDTH) - 1/WIDTH:
             img.put(color, (x,y))
        
    ram((0.3,0.5), (0.5,0.7), 2, "#ffa500")   
